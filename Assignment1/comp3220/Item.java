...
public class Item {
    ...
    public double getPrice(LocalDate sellDate){
        ...
        if(difference.getYears()>1 || (difference.getYears()==1 && difference.getDays()>=1)){
            if(this.discount==0)
                price=price*0.5;
            else
    ...
    public double sell(int amount, LocalDate sellDate){
        ...
        if(amount>0 && amount<=this.quantity){
            double salesValue=amount*this.regularPrice;
            if(this.discount!=0)
                salesValue*=this.discount;
    ...
    public Item makeCopy(){
        Item copy=new Item(this,this.getItemID());
        return copy;
    }
}
